...
async function getConfig(data, key) {
    let strFiltroLei = JSON.stringify([{ FielName: 'chave', Type: 'string', FixedType: 'string', Value1: key }]);
    let dataconfig = await getOnergyItem('efb11b9d-58d7-45fb-a8cd-d0ffbc707d0f', data.assid, data.usrid, strFiltroLei);
...
async function getMunicipioIbge(codMun) {
    let url = 'https://servicodados.ibge.gov.br/api/v1/localidades/municipios/' + codMun;
    ...
    await ajax({
        ...
        success: async function (resp) {
            ...
            if (re != null && re != '') {
                resultdata = { nome: re.nome, uf: re.microrregiao.mesorregiao.UF.sigla, err: false };
            }
        },
        error: async function (err) {
            resultdata = { err: true, msg: err };
        },
    });
    return resultdata;
}

async function getDataCepom(data, cnpj, uf, mun) {
    let strFiltroCacheSN = JSON.stringify([
        { FielName: 'cnpj', Type: 'string', Value1: cnpj },
        { FielName: 'municipio', Type: 'string', Value1: mun },
        { FielName: 'uf', Type: 'string', Value1: uf },
    ]);
    let result = await getOnergyItem('2220443a-6752-424b-a800-03993e6ae90c', data.assid, data.usrid, strFiltroCacheSN);
    ...
    if (result != null && result.length > 0) {
        ...
        if (diffDays >= 30) {
            ...
            return { cepom: result[0].UrlJsonContext.cpom, error: false, anexo: result[0].UrlJsonContext.url_comprovante, data: result[0].UrlJsonContext.data };
        }
    }
    let cnpjPrestador = '&cnpj=' + cnpj;
    let codMunTomad = '&uf=' + uf;
    let munTomador = '&municipio=' + mun;
    let resultTemCepom = false;
    let token = await getConfig(data, 'token_onergy_rpa');
    let urlCepom = 'https://api.infosimples.com/api/v1/pref/cpom.json?token=' + token + '&timeout=600' + cnpjPrestador + codMunTomad + munTomador;
    let urlAnexo = '';
    let error = false;
    ...
    await ajax({
        ...
        success: async function (resp) {
            if (resp != null && resp != '') {
                let infCepom = JSON.parse(resp);
                if (infCepom.data == undefined) {
                    if (infCepom.errors == undefined) {
                        resultTemCepom = infCepom.code_message;
                    } else resultTemCepom = infCepom.errors;
                    error = true;
                    return;
                }
                dataresult = infCepom.data;
                resultTemCepom = !(infCepom.data.normalizado_situacao == 'inativo'); //TODO confuso
                //urlAnexo = infCepom.receipt.sites_urls[0];
                urlAnexo = UploadToFile(infCepom.receipt.sites_urls[0]);
                if (!ExistInCache) {
                    let data2 = {
                        cnpj: cnpj,
                        cpom: resultTemCepom,
                        ...
                        municipio: mun,
                        uf: uf,
                        url_comprovante: urlAnexo,
                        data_pesquisa: new Date(),
                    };
                    let saveresult = await onergy_save({
                        assid: data.assid,
                        usrid: data.usrid,
                        fdtid: '2220443a-6752-424b-a800-03993e6ae90c',
                        ukField: 'cnpj',
    ...
    return { cepom: resultTemCepom, anexo: urlAnexo, error: error, data: dataresult };
}

async function init(json) {
    const data = JSON.parse(json);
    data.ibge = await getMunicipioIbge(data.munTomador);
    if (data.ibge != undefined) {
        let resultcepom = await getDataCepom(data, data.cnpj, data.ibge.uf, data.ibge.nome);
        data.cepom = resultcepom.cepom;
        if (resultcepom.error) {
            data.cadastro_no_cpom = 'PROBLEMA NA PESQUISA DE ORIGEM';
        } else data.cadastro_no_cpom = resultcepom.cepom == true ? 'Cadastrado no CEPOM' : 'INATIVO';
        if (resultcepom.anexo != '') {
            if (data.anexos == undefined) {
                data.anexos = [];
                data.anexos.push({ Name: 'Cepom.html', UrlAzure: resultcepom.anexo, Url: resultcepom.anexo });
            } else {
                let notfound = true;
                for (let anexo in data.anexos) {
                    if (data.anexos[anexo].Name == 'Cepom.html') {
                        data.anexos[anexo].UrlAzure = resultcepom.anexo;
                        data.anexos[anexo].Url = resultcepom.anexo;
                        notfound = false;
                    }
                }
                if (notfound) data.anexos.push({ Name: 'Cepom.html', UrlAzure: resultcepom.anexo, Url: resultcepom.anexo });
            }
        }
    }
